...
static const std::string filename = "C:/Programacion Paralela/correccion_cuda/lobo.jpeg";
...
void rotate_image(const sf::Uint8 *image) {
    ...
    CHECK(cudaMemcpy(device_src_image_pixels, image, buffer_size, cudaMemcpyHostToDevice));

    kernel_rotar(device_src_image_pixels, device_blur_image_pixels, image_width, image_height, blur_dimension / 2);

    CHECK(cudaGetLastError());

    //copiar imagen device==>host
    CHECK(cudaMemcpy(image_rotated.data(), device_blur_image_pixels, buffer_size, cudaMemcpyDeviceToHost));
}

void save_image() {
    auto fname = fmt::format("output-blur-{}.png", blur_dimension);
    sf::Image im;
    im.create(image_width, image_height, image_rotated.data());
    im.saveToFile(fname);
}
...
int main() {
    ...
    image_rotated.resize(buffer_size);
        ...
        while (window.pollEvent(event)) {
            if (event.type == sf::Event::Closed) {
                ...
                if (event.key.scancode == sf::Keyboard::Scan::Up) {
                    ...
                    texture.update(image_rotated.data());
                }
